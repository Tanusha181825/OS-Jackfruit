...
static void timer_callback(struct timer_list *t) {
    struct monitored_node *entry, *tmp;
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        if (rss > entry->hard_limit) {
            ...
            rcu_read_unlock();
            printk(KERN_WARNING "[container_monitor] HARD LIMIT KILL container=%s pid=%d rss=%ld\n", entry->container_id, entry->pid, rss);
            list_del(&entry->list);
            kfree(entry);
        } else if (rss > entry->soft_limit && !entry->soft_warned) {
            printk(KERN_WARNING "[container_monitor] SOFT LIMIT WARNING container=%s pid=%d rss=%ld\n", entry->container_id, entry->pid, rss);
...
static void __exit monitor_exit(void) {
    ...
    mutex_lock(&monitor_lock);
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) { list_del(&entry->list); kfree(entry); }
    mutex_unlock(&monitor_lock);
    cdev_del(&c_dev);
